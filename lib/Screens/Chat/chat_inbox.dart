...
// A global map to store messages for each chat session.
// In a real app, this would be a database.
Map<int, List<LMSInboxData>> chatMessages = {};
...
class ChatInbox extends StatefulWidget {
  ...
  const ChatInbox({this.img, this.name, required this.instructorId});
...
class _ChatInboxState extends State<ChatInbox> {
  ...
  List<LMSInboxData> data = [];

  @override
  void initState() {
    super.initState();
    init();
    // Initialize the message list for this specific chat
    chatMessages.putIfAbsent(widget.instructorId, () => []);
    data = chatMessages[widget.instructorId]!;
  }
      ...
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            CircleAvatar(
              radius: 20,
              backgroundImage: AssetImage(widget.img.validate()),
            ),
            8.width,
            Column(
  ...
  void addMessage() {
    ...
    setState(() {
      // Add message to the specific chat list
      chatMessages[widget.instructorId]!.insert(
        0,
        LMSInboxData(id: 0, message: messageController.text),
      );
